...
pub fn ocr_recognize_batch(_app: AppHandle, image_path: String) -> Result<OcrBatchResult, String> {
    info!(
        "[ocr_recognize_batch] 开始批量OCR识别: image_path={}",
        image_path
    );
    ...
    info!(
        "[ocr_recognize_batch] OCR批量识别成功: {} 条账单, 总金额={}, 日期={}",
        items.len(),
        result.total_amount,
        result.bill_date
    );
    debug!(
        "[ocr_recognize_batch] 原始文本长度: {} 字符",
        result.raw_text.len()
    );

    // 根据识别出的日期提取账单月份 YYYY-MM
...
pub fn save_bill_with_image(
    ...
    // 计算账单月份：优先使用传入的 bill_month，否则从 bill_date 提取
    ...
    info!(
        "[save_bill_with_image] 账单和图片保存完成, bill_id={}",
        bill_id
    );
    Ok(bill_id)
}
